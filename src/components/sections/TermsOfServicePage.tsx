...
const TermsOfServicePage = () => {
  const lastUpdated = "25/04/2026";
  const sections = termsOfServiceSections as TermsSection[];
  const additionalTerms = termsOfServiceAdditionalTerms as {
    number: string;
    title: string;
    content: string;
  }[];
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (heroRef.current) {
        ...
        const metaItems = heroRef.current.querySelectorAll(
          ".lg\\:col-span-4 > div",
        );
      ...
      if (additionalRef.current) {
        const termItems =
          additionalRef.current.querySelectorAll(":scope > div > div");
      ...
      if (footerRef.current) {
        const footerChildren =
          footerRef.current.querySelectorAll(":scope > div");
    ...
    <div className="min-h-screen bg-white text-black">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12">
          ...
          <div className="lg:col-span-8 p-6 sm:p-10 lg:p-16 border-b-2 lg:border-b-0 lg:border-r-2 border-black">
            ...
            <p className="text-sm sm:text-base lg:text-lg text-black/60 max-w-xl leading-relaxed">
              ...
              RESQ. Bằng việc sử dụng ứng dụng, bạn xác nhận đã đọc, hiểu và
          ...
          <div className="lg:col-span-4 flex flex-col">
            ...
            <div className="p-6 sm:p-10 lg:p-12 bg-black text-white flex-1">
              <span className="text-[10px] font-mono tracking-widest text-white/40 block mb-2">
                PHIÊN BẢN
              </span>
              <span className="text-2xl sm:text-3xl lg:text-4xl font-black">
                V1.0
      ...
      <section ref={sectionsRef}>
        {sections.map((section, index) => (
          <div
            ...
            className={`border-b-2 border-black ${section.isEmergency ? "bg-[#FF5722] text-white" : section.isDark ? "bg-black text-white" : ""}`}
          >
            <div className="grid grid-cols-1 lg:grid-cols-12">
              {/* Number Column */}
              <div
                className={`lg:col-span-2 p-6 sm:p-8 lg:p-12 border-b-2 lg:border-b-0 lg:border-r-2 ${section.isEmergency ? "border-white/30" : section.isDark ? "border-white/20" : "border-black"}`}
              >
                <span
                  className={`text-6xl sm:text-7xl lg:text-8xl font-black ${section.isEmergency || section.isDark ? "text-white/20" : "text-black/10"}`}
                >
                  {section.number}
                </span>
              </div>
              ...
              <div className="lg:col-span-10 p-6 sm:p-8 lg:p-12">
                <h2
                  className={`text-xl sm:text-2xl lg:text-3xl font-black tracking-tight mb-6 ${section.isEmergency || section.isDark ? "text-white" : ""}`}
                >
                  {section.title}
                </h2>
                ...
                <div className="space-y-4">
                  {section.content.map((text, i) => (
                    <p
                      key={i}
                      className={`text-sm sm:text-base lg:text-lg leading-relaxed ${section.isEmergency || section.isDark ? "text-white/80" : "text-black/70"}`}
                    >
                      {text}
                    </p>
                  ))}
                    ...
                    {section.list.map((item, i) => (
                      <div
                        ...
                        className={`p-4 transition-colors group ${
                          section.isEmergency
                            ? "border-2 border-white/40 hover:bg-white hover:text-black"
                            : section.isDark
                              ? "border-2 border-white/20 hover:bg-white hover:text-black"
                              : "border-2 border-black hover:bg-black hover:text-white"
                        }`}
                      >
                        <span
                          className={`text-xs font-black tracking-wider block mb-1 ${
                            section.isEmergency || section.isDark
                        ...
                        <span
                          className={`text-xs ${
                            section.isEmergency || section.isDark
                              ? "text-white/60 group-hover:text-black/50"
        ...
        <section ref={additionalRef} className="border-b-2 border-black">
          <div className="grid grid-cols-1 sm:grid-cols-2">
            {additionalTerms.map((term, index) => (
              <div
                key={index}
                className={`p-6 sm:p-8 lg:p-12 border-b-2 sm:border-b-0 ${index % 2 === 0 ? "sm:border-r-2" : ""} ${index < additionalTerms.length - 2 ? "sm:border-b-2" : ""} border-black`}
              >
      ...
      <section className="border-t-2 border-black bg-black/5 p-6 sm:p-8">
        <p className="text-xs sm:text-sm text-center text-black/60 max-w-3xl mx-auto">
          Bằng việc sử dụng ứng dụng ResQ SOS Miền Trung, bạn xác nhận rằng bạn
          đã đọc, hiểu và đồng ý với tất cả các điều khoản và điều kiện được nêu
          trong tài liệu này.
        </p>
      </section>
    </div>
