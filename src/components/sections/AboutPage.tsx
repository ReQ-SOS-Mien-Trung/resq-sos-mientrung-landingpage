...
import TeamMemberThreeModel from "./TeamMemberThreeModel";

gsap.registerPlugin(ScrollTrigger);

const TEAM_FORMAL_BOW_MODEL_BY_NAME: Record<string, string> = {
  "Huỳnh Kim Cương": "/models/KimCu/KimCu_Formal_Bow.glb",
  "Lê Minh Đăng Khoa":
    "/models/Khoalu/Meshy_AI_Tropical_Print_Ensemb_biped_Animation_Formal_Bow_withSkin.glb",
  "Nguyễn Trần Phương An":
    "/models/PhuongTuan/Meshy_AI_Violet_Lotus_Ao_Dai_biped_Animation_Formal_Bow_withSkin.glb",
  "Nguyễn Ngọc Thảo":
    "/models/ChiE/Meshy_AI_Executive_Portrait_biped_Animation_Formal_Bow_withSkin.glb",
  "Lê Bảo Châu": "/models/BaoChu/Meshy%20AI_BaoChua_Formal_Bow.glb",
};

const TEAM_BUNG_BO_MODEL_BY_NAME: Record<string, string> = {
  "Huỳnh Kim Cương": "/models/KimCu/KimCuDance.glb",
  "Lê Minh Đăng Khoa":
    "/models/Khoalu/Meshy_AI_Tropical_Print_Ensemb_biped_Animation_Breakdance_1990_withSkin.glb",
  "Nguyễn Trần Phương An":
    "/models/PhuongTuan/Meshy_AI_Violet_Lotus_Ao_Dai_biped_Animation_Hip_Hop_Dance_2_withSkin.glb",
  "Nguyễn Ngọc Thảo":
    "/models/ChiE/Meshy_AI_Executive_Portrait_biped_Animation_Step_Hip_Hop_Dance_withSkin.glb",
  "Lê Bảo Châu": "/models/BaoChu/Meshy_AI_BaoChua.glb",
};
...
const AboutPage = () => {
  ...
  const [showTeamModels, setShowTeamModels] = useState(false);
  const [isBungBoMode, setIsBungBoMode] = useState(false);
    ...
    <div className="min-h-screen bg-white text-black overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2">
          ...
          >
            ...
            <p className="hero-anim text-sm sm:text-base lg:text-lg text-black/60 max-w-xl leading-relaxed mb-8">
              ResQ SOS Miền Trung là nền tảng công nghệ kết nối người dân gặp
              nạn trong thiên tai với đội ngũ cứu hộ tình nguyện, giúp việc cứu
              trợ diễn ra nhanh chóng và hiệu quả hơn.
            </p>
            <Link
              to="/register"
          ...
          <div ref={statsRef} className="grid grid-cols-2">
            {aboutStats.map((stat, index) => (
              ...
              >
                <span className="text-3xl sm:text-4xl lg:text-5xl font-black text-[#FF5722]">
                  {stat.number}
                </span>
                <span className="text-[10px] font-mono tracking-widest text-black/50 mt-2">
                  {stat.label}
                </span>
              </div>
            ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12">
          ...
          <div className="mission-anim lg:col-span-8 p-8 sm:p-12 lg:p-16">
            <p className="text-base sm:text-lg lg:text-xl text-black/70 leading-relaxed mb-6">
              Chúng tôi tin rằng trong những thời khắc khó khăn nhất, công nghệ
              có thể trở thành cầu nối giữa những người cần giúp đỡ và những tấm
              lòng sẵn sàng hỗ trợ.
            </p>
            <p className="text-sm sm:text-base text-black/60 leading-relaxed">
              ResQ SOS Miền Trung ra đời với sứ mệnh đảm bảo mọi người dân trong
              vùng thiên tai đều có thể tiếp cận được sự trợ giúp kịp thời.
              Chúng tôi xây dựng một hệ sinh thái cứu hộ thông minh, nơi công
              nghệ AI giúp phân loại mức độ khẩn cấp, điều phối lực lượng cứu hộ
      ...
      <section ref={valuesRef} className="border-b-2 border-black">
        <div className="p-8 sm:p-12 lg:p-16 border-b-2 border-black">
          <span className="text-[10px] font-mono tracking-widest text-black/40 block mb-2">
            GIÁ TRỊ CỐT LÕI
          </span>
          ...
          {aboutValues.map((value, index) => (
            ...
            >
              <value.icon
                ...
                weight="fill"
              />
              <h3 className="text-sm font-black tracking-wider mb-2">
                {value.title}
              </h3>
      ...
      <section id="team" ref={teamRef} className="border-b-2 border-black">
        <div className="p-8 sm:p-12 lg:p-16 border-b-2 border-black">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="text-2xl sm:text-3xl font-black">
                NHỮNG NGƯỜI ĐỨNG SAU RESQ
              </h2>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <button
                type="button"
                onClick={() => setShowTeamModels((prev) => !prev)}
                className="self-start sm:self-auto px-4 py-2 border-2 border-black text-xs font-black tracking-wider uppercase hover:bg-black hover:text-white transition-colors"
                aria-pressed={showTeamModels}
              >
                {showTeamModels ? "Tắt model 3D" : "Bật model 3D"}
              </button>
              <button
                type="button"
                onClick={() => setIsBungBoMode((prev) => !prev)}
                className={`self-start sm:self-auto px-4 py-2 border-2 text-xs font-black tracking-wider uppercase transition-colors ${isBungBoMode ? "border-[#FF5722] bg-[#FF5722] text-white" : "border-black text-black hover:bg-black hover:text-white"} disabled:opacity-40 disabled:cursor-not-allowed`}
                aria-pressed={isBungBoMode}
                disabled={!showTeamModels}
                title={
                  !showTeamModels
                    ? "Bật model 3D để dùng chế độ bùng bổ"
                    : "Chuyển sang model bùng bổ"
                }
              >
                Bùng bổ
              </button>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5">
          {aboutTeam.map((member, index) => {
            const formalModelSrc = TEAM_FORMAL_BOW_MODEL_BY_NAME[member.name];
            const bungBoModelSrc = TEAM_BUNG_BO_MODEL_BY_NAME[member.name];
            const selectedModelSrc = isBungBoMode
              ? (bungBoModelSrc ?? formalModelSrc)
              : formalModelSrc;
            ...
            return (
              ...
              >
                ...
                {showTeamModels && selectedModelSrc && (
                  <TeamMemberThreeModel
                    src={selectedModelSrc}
                    alt={`${member.name} ${isBungBoMode ? "Bung bo" : "Formal Bow"}`}
                  />
                )}
              </div>
            );
      ...
      <section ref={ctaRef} className="grid grid-cols-1 lg:grid-cols-2">
        <div className="cta-anim p-8 sm:p-12 lg:p-16 bg-[#FF5722] text-white border-b-2 lg:border-b-0 lg:border-r-2 border-black">
          ...
          <p className="text-sm text-white/80 mb-6">
            Trở thành một phần của cộng đồng cứu hộ. Mỗi đóng góp của bạn đều có
            ý nghĩa.
          </p>
          <Link
            to="/register"
        ...
        <div className="cta-anim p-8 sm:p-12 lg:p-16">
          ...
          <div className="space-y-3 text-sm">
            ...
            <p>
              <span className="text-black/50">Địa chỉ:</span> 123 Đường Trần
              Phú, TP. Đà Nẵng
            </p>
          </div>
        </div>
